...
int main(int argc, char **argv)
{
	...
	printSystem(stdout);
	#endif
	size_t size = NUMENTITIES * NUMENTITIES * sizeof(vector3);
	// allocate memory for the accels array on the GPU
	cudaMalloc(&accels, size);
	cudaMalloc(&d_hPos, sizeof(vector3) * NUMENTITIES);
	cudaMemcpy(d_hPos, hPos, sizeof(vector3) * NUMENTITIES, cudaMemcpyHostToDevice);

	// make a mass to use on the device
	...
	// copy the fields back from GPU to CPU!
